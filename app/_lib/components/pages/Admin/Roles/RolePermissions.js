import { addPermissionToRole, getRolePermissions, removePermissionFromRole } from "@/actions/roleActions"
import Spinner from "@/components/atoms/Spinner/Spinner"
import Tooltip from "@/components/molecules/Tooltip/Tooltip"
import Plus from "@/public/svgs/plus"
import Close from "@/public/svgs/close-line"
import { useEffect, useState } from "react"

export default function RolePermissions( { role, availablePermissions } ) {
    ...
    useEffect(() => {
        const fetchRolePermissions = async () => {
            const userPermissions = await getRolePermissions(role?.id) 
    ...
    const removePermission = (p) => {
        ...
        removePermissionFromRole(role?.id, p.id)
    }

    const addPermission = (p) => {
        setPermissions([...permissions, p])
        addPermissionToRole(role?.id, p.id)
    }
